...
search.addEventListener('input', (e) => {
  ...
  // ensuite je vide la liste (ul) pour ne garder que le/les résultat(s) de la recherche
...
mangaData
  ...
  .forEach(manga => { // boucle foreach, pour chaque manga 
...
mangaData
  ...
  .forEach((manga) => {
    ...
    latestReviewsCards.appendChild(cardElement); // j'ajoute à la liste bestRatedList ma variable cardElement
  });


// fonction qui affiche les manga que je recommande à lire
...
mangaData.filter((manga) => {
  ...
  .forEach((manga) => {
     ...
     let cardElement = getMangaCard(manga, false, false, true); // je créer la variable cardElement qui prends l'appel de la fonction getTopManga(manga)
